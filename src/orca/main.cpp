...
static OrcaConfig parseOrcaToml(const std::string& path) {
	...
	while (std::getline(f, line)) {
		...
		if (section == "profile.debug") {
			if (key == "opt") cfg.debugOpt = val;
		}
		if (section == "profile.release") {
			if (key == "opt") cfg.releaseOpt = val;
		}
		if (section == "build") {
			if (key == "opt")    cfg.buildOpt = val;
...
static CodeGen::OptLevel parseOptLevel(const std::string& s) {
	if (s == "1") return CodeGen::OptLevel::O1;
	if (s == "2") return CodeGen::OptLevel::O2;
	if (s == "3") return CodeGen::OptLevel::O3;
	if (s == "s") return CodeGen::OptLevel::Os;
	if (s == "z") return CodeGen::OptLevel::Oz;
...
static void writeCache(const std::string& cachePath,
...
static int cmdClean(const std::string& tomlPath, bool all) {
	...
	}
	{
		...
		if (slashPos != std::string::npos) projectRoot = tomlPath.substr(0, slashPos);
	}

	std::error_code ec;
	auto canonRoot = std::filesystem::canonical(projectRoot, ec);
	if (ec) {
		std::cerr << "[orca] Error: Cannot resolve project root '" << projectRoot
		          << "': " << ec.message() << "\n";
		return 1;
	}
	...
	auto safeRemove = [&](const std::string& pathStr) {
		...
		if (ec) std::cerr << "[orca] Warning: Failed to remove " << pathStr << ": " << ec.message() << "\n";
		else    std::cout << "[orca] Removed: " << pathStr << "\n";
	};
	...
	const std::string exeName = cfg.output.empty()
	    ? (cfg.name.empty() ? "" : cfg.name) : cfg.output;
	if (!exeName.empty()) {
...
static int cmdCheck(const std::string& tomlPath) {
	try {
		...
		for (const auto& mod : resolver.allModulePaths()) {
			...
			if (!file.empty()) modToFile[mod] = file;
		}
		...
		for (const auto& [mod, file] : modToFile) {
			if (!checkedFiles.insert(file).second) continue;  // skip duplicate file paths
			try {
				parseFile(file);
			} catch (const ParseError& e) {
				std::cerr << file << ":" << e.line << ":" << e.column
				          << ": error: " << e.what() << "\n";
				++errorCount;
...
int main(int argc, char* argv[]) {
	...
	for (int i = 1; i < argc; ++i) {
		if (doRun && std::strcmp(argv[i], "--") == 0) {
			...
			for (int j = i + 1; j < argc; ++j) runArgs.push_back(argv[j]);
			break;
		}
		if (std::strcmp(argv[i], "--project") == 0 && i + 1 < argc) {
			tomlPath = argv[++i];
	...
	try {
		...
		if (outputFile.empty()) {
			...
			if (!cfg.output.empty()) outputFile = cfg.output;
			else outputFile = cfg.name.empty() ? "a.out" : cfg.name;
		}
		...
		for (const auto& mod : sorter.order) {
			...
			if (!emitIR) {
				...
				if (!ts.empty() && cit != cache.end() && cit->second == ts
				    && std::filesystem::exists(objPath)) {
			...
			try {
				...
				if (emitIR) {
					...
					std::string ts = fileTimestamp(file);
					if (!ts.empty()) { cache[file] = ts; cacheUpdated = true; }
				}
		...
		if (cacheUpdated) writeCache(cacheFile, cache);
		...
		if (doRun) {
			...
			for (const auto& a : runArgs) runArgv.push_back(a.c_str());
			runArgv.push_back(nullptr);
			execvp(execPath.c_str(), const_cast<char* const*>(runArgv.data()));
			// execvp only returns on error.
			std::cerr << "[orca] Failed to execute '" << execPath
			          << "': " << strerror(errno) << "\n";
			return 1;
		}
