// Linux / ELF linker implementation.
// Compiled on non-Windows hosts only — see CMakeLists.txt.
...
void Linker::link(const Config& config) {
	...
	// Resolve output filename: default to "a.out" on Linux.
	std::string outputFile = config.outputFile.empty() ? "a.out" : config.outputFile;
	...
	// Convert to vector<StringRef> for LLVM API.
